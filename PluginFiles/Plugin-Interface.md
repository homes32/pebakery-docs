...
The Interface section contains one ore more lines that define the controls of a Plugin's Graphical User Interface. 

## Controls

The following control types are support by PEBakery. Click on the control's name for details on how to use each control.
...
| 14 | [Radio Group](./RadioGroup.md) | A group of radio buttons. Only one button per group may be slected at a time. |
